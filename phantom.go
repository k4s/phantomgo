...
func (self *Phantom) Start(args []string) (result io.ReadCloser, err error) {
	if len(args) == 2 {
		...
		if args[0] == "exec" {
			...
			result, err = self.Open(GOPATH+"/src/github.com/k4s/phantomgo/js/open.js", args[1])
			if err != nil {
				return nil, err
			}
		...
		if args[0] == "open" {
			...
			if self.userAgent != "" {
				...
				result, err := self.Open(GOPATH+"/src/github.com/k4s/phantomgo/js/open.js", args[1], args[2], self.userAgent)
				if err != nil {
					return nil, err
				}
				return result, err
			} else {
				result, err := self.Open(GOPATH+"/src/github.com/k4s/phantomgo/js/open.js", args[1], args[2])
				if err != nil {
					return nil, err
				}
				return result, err
...
func (self *Phantom) Open(openArgs ...string) (stdout io.ReadCloser, err error) {
	cmd := exec.Command("phantomjs", openArgs...)
